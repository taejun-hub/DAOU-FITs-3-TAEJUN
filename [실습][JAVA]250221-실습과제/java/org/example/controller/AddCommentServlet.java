...
public class AddCommentServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int success = this.commentServletService.addComment(new CommentVO(content, Integer.parseInt(board_id), memberId));
        if (success > 0) {
            jsonResponse.put("status", "success");
            jsonResponse.put("message", "댓글이 등록되었습니다.");
            jsonResponse.put("boardId", board_id);
            jsonResponse.put("commentContent", content);
        } else {
